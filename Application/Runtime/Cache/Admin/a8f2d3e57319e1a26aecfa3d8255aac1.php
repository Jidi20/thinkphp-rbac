			...
			<div class="result-title">
				<div class="result-list">
					<div class="result-list-left">
						<a href="/index.php/Admin/Module/add"><input type="button" class="butstyle mr10 bg1" value="新增模块"></a>
					</div>
			...
			<div class="result-content">
				<table class="result-tab" width="100%">
					...
					<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
							...
							<td><?php if($vo["is_view"] == 1): ?><span class="c1">是</span><?php else: ?><span class="c2">否</span><?php endif; ?></td>
							<td><?php echo ($vo["sort"]); ?></td>
							<td>
								<a href="/index.php/Admin/Module/edit/id/<?php echo ($vo["id"]); ?>" class="c1">修改</a> |
								<a href="javascript:;" class="c1" onclick="delete_one('<?php echo ($vo["id"]); ?>');">删除</a>
							</td>
	...
	<script type="text/javascript">
		...
		function delete_one(id){
			Boxy.confirm("确定删除？", function() {
				$.post('/index.php/Admin/Module/delete_one',{id:id},function(data){
					if(data.success == 1){
						Boxy.alert(data.message,function(){
							location.href='/index.php/Admin/Module/index';
						});return false;
		...
		function delete_all(){
			...
			if(ids != ""){
				Boxy.confirm("确定删除？", function() {
					$.post("/index.php/Admin/Module/delete_all",{ids:ids},function(data){
						if(data.success == 1){
							location.href="/index.php/Admin/Module/module_list";
						}else{
							alert(data.message);
						}
			    	},'json');
		...
		function search(){
			...
			var url = '/index.php/Admin/Module/index';
			if(keyword != ''){
				url += '/keyword/'+keyword;
			}
			location.href = url;
