...
<div class="container clearfix global" id="container_height">
						...
						<tr>
							...
							<td>
								<select name="pid" id="" class="selstyle w300">
									...
									<?php if(is_array($list)): foreach($list as $key=>$vo): if($vo['o_level'] != 3): ?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["html"]); echo ($vo["name"]); ?></option><?php endif; endforeach; endif; ?>
								</select>
							</td>
						</tr>
						<tr>
	...
	<script type="text/javascript">
		...
		if(add_status){
			add_status=false;
			$.ajax({
				url :"/index.php/Admin/Module/add_ajax",
				type : 'post',
