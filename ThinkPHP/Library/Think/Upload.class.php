...
class Upload {
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            $getimginfo = getimagesize($file['tmp_name']);
            $w = $this->config['maes'];
            if(!empty($w)){
                foreach($w as $k => $v){
                    $la = $k == 0 ? "宽度" : "高度";
                    if(is_array($v)&&!empty($v)){
                        if($v[0] == "egt" && $getimginfo[$k]<$v[1]){
                            array_push($error,$file['name']."图片{$la}不能小于{$v[1]}像素");
                            continue;
                        }else if($v[0] == "gt" && $getimginfo[$k]<=$v[1]){
                            array_push($error,$file['name']."图片{$la}应大于{$v[1]}像素");
                            continue;
                        }else if($v[0] == "elt" && $getimginfo[$k]>$v[1]){
                            array_push($error,$file['name']."图片{$la}不能大于{$v[1]}像素");
                            continue;
                        }else if($v[0] == "lt" && $getimginfo[$k]>=$v[1]){
                            array_push($error,$file['name']."图片{$la}应小于{$v[1]}像素");
                            continue;
                        }
    ...
    private function getSaveName($file) {
        ...
        $this->savename = $savename . '.' . $ext;
        return $savename . '.' . $ext;
    }
    
    public function _getSaveName(){
