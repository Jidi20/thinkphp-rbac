 ...
 ********************** */
var afteruploadSuccess;
var afteruploadError;
...
function fileQueueError(file, errorCode, message) {
    try {
        this.customSettings.queueError = true;
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
    try {
        if (numFilesSelected > 0 && this.customSettings.queueError === false) {
            var cancelButtonIdLen = $("*[id=" + this.customSettings.cancelButtonId + "]").size();
            if (cancelButtonIdLen == 1)
                document.getElementById(this.customSettings.cancelButtonId).disabled = false;
            else {
                $("object[zw=" + this.settings.button_placeholder_id + "]").closest("div").find("#" + this.customSettings.cancelButtonId).get(0).disabled = false;
            }
...
function uploadSuccess(file, serverData) {
    try {
        ...
        progress.toggleCancel(false);
        if(Ajax.isFunction(afteruploadSuccess)){
            afteruploadSuccess(serverData,file);
        }
    } catch (ex) {
        this.debug(ex);
    }
    ...
    try {
        ...
        switch (errorCode) {
            ...
            case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                progress.setStatus("超出上传限制:"+message+".");
            ...
            case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
                ...
                break;
            default:
                progress.setStatus("未处理的错误: " + errorCode+","+message+".");
                this.debug("错误代码: " + errorCode + ", 文件名: " + file.name + ", 文件尺寸: " + file.size + ", 信息: " + message);
                break;
        }
        if(Ajax.isFunction(afteruploadError)){
            afteruploadError(file);
        }
    } catch (ex) {
        this.debug(ex);
    }
...
function uploadComplete(file) {
    if (this.getStats().files_queued === 0) {
        ...
        if (cancelButtonIdLen == 1)
            ...
            $("object[zw=" + this.settings.button_placeholder_id + "]").closest("div").find("#" + this.customSettings.cancelButtonId).get(0).disabled = true;
        }
    }
}
...
function queueComplete(numFilesUploaded) {
    var divStatusLen = $("*[id=divStatus]").size();
    if (divStatusLen == 1)
        var status = document.getElementById("divStatus");
    else {
        var status = $("object[zw=" + this.settings.button_placeholder_id + "]").closest("div").find("#divStatus").get(0);
    }
    //status.innerHTML = numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : "s") + "已上传.";
    var fail = parseInt(numFilesUploaded) - parseInt(this.customSettings.successFile);
    status.innerHTML = "共上传 "+numFilesUploaded + " 个文件,成功 " +  parseInt(this.customSettings.successFile) + " 个,失败 "+fail+" 个";
}
